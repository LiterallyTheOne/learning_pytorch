...
def prepare_and_get_data_path(where_is_running: str = 'colab') -> Path:
    match where_is_running:
        case 'colab':
            ...
            os.system('kaggle datasets download -d likhon148/animal-data')
            os.system('unzip -q animal-data.zip -d animal-data')
            os.system('rm -r animal-data.zip')

            data_path = Path('animal-data')

        case 'pc':
            data_path = Path('/home/ramin/ramin_programs/files/datasets/animal_data')
